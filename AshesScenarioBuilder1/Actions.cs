...
namespace AshesScenarioBuilder1
{
    ...
    public class Action
    {
        ...
        public static bool isStillAlive(string name, Trigger currentTrigger, bool isBuilding)
        {
            Trigger[] prior = currentTrigger.getAllPriorTriggers();
            if (prior == null
                || (isBuilding && Trigger.findDestroyBuilding(prior, name) != null)
                || (!isBuilding && Trigger.findDestroyUnit(prior, name) != null))
                return false;
            return true;
        }


        public ActivateAI toActivateAI()
        {
            return new ActivateAI(trig, getStringA(), getIntA());
        }
        public ActivateTrigger toActivateTrigger()
        {
            return new ActivateTrigger(trig, getStringA());
        }
        public SpawnUnit toSpawnUnit()
        {
            return new SpawnUnit(trig, getStringA(), getStringB(), getStringC(), getIntA(), new Position(getPositionAX(), getPositionAY(), getPositionAZ()), getBoolA());
        }
        public SpawnBuilding toSpawnBuilding()
        {
            return new SpawnBuilding(trig, getStringA(),getStringB(), getIntA(), new Position(getPositionAX(), getPositionAY(), getPositionAZ()));
        }
        public AreaIndicator toAreaIndicator()
        {
            return new AreaIndicator(trig, getStringA(), getStringB(), getIntA(), new Position(getPositionAX(), getPositionAY(), getPositionAZ()), getBoolA());
        }
        public Objective toObjective()
        {
            return new Objective(trig, getStringA(), getStringB(), getBoolA());
        }

        public DestroyUnit toDestroyUnit()
        {
            return new DestroyUnit(trig, getStringA(), getIntA());
        }

        public DestroyBuilding toDestroyBuilding()
        {
            return new DestroyBuilding(trig, getStringA(), getIntA());
        }

        public AttackAttackMove toAttackAttackMove()
        {            
            return new AttackAttackMove(trig, getStringA(), new Position(getPositionAX(), getPositionAY(), getPositionAZ()));
        }
    }
    ...
    public class ActivateTrigger : Action
    {
        ...
        /// <summary>
        /// Trigger to be activated by this action
        /// </summary>
    ...
    public class DialogEntry : Action
    {
        ...
        /// <summary>
        /// The picture used by the GUI to show the icon
        /// </summary>
    ...
    public class AreaIndicator : Action
    {
        ...
        /// <summary>
        /// Deduces if the unit spawned by this action has been destroyed by a destroyUnit action by the given Trigger  
        /// </summary>
        /// <param name="currentTrigger">The Trigger to be considered the "present"</param>
        /// <returns>True if the unit may be still active, false if the unit is certainly destroyed</returns>
        public bool isStillActive(Trigger currentTrigger)
        {
            if (!on) return false;
            Trigger[] prior = currentTrigger.getPriorTriggers();
            foreach(Trigger curTrigger in prior)
                foreach(Action curAction in currentTrigger.actions)
                    if (curAction.GetType().Equals(typeof(AreaIndicator))
                        &&curAction.getStringA().ToLower().Equals(name.ToLower())
                        &&!curAction.getBoolA())
                            return false;                           
            return true;
        }
    ...
    public class SpawnUnit : Action
    {
        ...
        /// <summary>
        ///  Returns whether or not the spawned unit has been destroyed by an action by the given trigger
        /// </summary>
        /// <param name="currentTrigger">The trigger considered to be the "present"</param>
        /// <returns>True if the unit has not been destryed by an action</returns>
        public bool isStillAlive(Trigger currentTrigger)
        {
            Trigger[] prior = currentTrigger.getAllPriorTriggers();
            if (prior == null && Trigger.findDestroyUnit(prior, name) != null)
                return false;
            return true;
        }
    }
    ...
    public class SpawnBuilding : Action
    {
        ...
        {
            Trigger[] prior = currentTrigger.getAllPriorTriggers();
            if (prior == null || Trigger.findDestroyBuilding(prior, name) != null)
                return false;
            return true;
        }
    }
