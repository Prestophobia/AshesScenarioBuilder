...
namespace AshesScenarioBuilder1
{
    ...
    public class Scenario
    {
        ...
        {
            Trigger[] output= new Trigger[0];
            if(triggers!=null)foreach(Trigger cur in triggers)
        ...
        public SpawnUnit findSpawnUnit(string name)
        {
            ...
            {
                name = name.ToLower();
                if(triggers!=null)foreach (Trigger curTrigger in triggers)
        ...
        public int getPlayerColor(int pNumber)
        {
            if(pNumber>=0&&pNumber<players.Length)
            {
                return players[pNumber].color;
            }
            return -1;
        }

        public bool isInScenario(Trigger t)
        {
            if (triggers != null)
                foreach (Trigger curTrig in triggers)
                    if (curTrig == t) return true;
            return false;
        }
